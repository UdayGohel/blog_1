...
<body>
    ...
    <header class="catagory__title">
        <h2>{{ $c->title }}</h2>
    </header>

    {{-- =======================  END OF CATAGORY TITLE ====================== --}}
            ...
            @foreach ($b as $cpost)
                <article class="post">
                    <div class="post__thumbail">
                        <img src="{{ asset('storage/images/thumbnail/' . $cpost->thumbnail) }}" alt="">
                    </div>
                    <div class="post__info">
                        <a href="category-posts.html" class="category__button">{{ $cpost->title }}</a>
                        <h3 class="post__title"><a href="post.html">{{ $cpost->post_title }}</a>
                        </h3>
                        <p class="post__body">{{ $cpost->body }}</p>
                        <div class="post__author">
                            <div class="post__author__avatar">
                                <img src="{{ asset('storage/images/' . $cpost->avatar_src) }}" alt="">
                            </div>
                            <div class="post__author-info">
                                <h5>By: {{ $cpost->firstname }}</h5>
                                <small>{{ $cpost->updated_at }}</small>
                            </div>
                        </div>
                    </div>
                </article>
